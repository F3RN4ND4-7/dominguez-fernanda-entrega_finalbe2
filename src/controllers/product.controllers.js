...
export const create = async (req, res, next) => {
  try {
    ...
    if (!newProd) res.status(404).json({ msg: "Error creando producto!" });
...
export const update = async (req, res, next) => {
  try {
    ...
    if (!prodUpd) res.status(404).json({ msg: "Error actualizando el producto!" });
...
export const remove = async (req, res, next) => {
  try {
    ...
    if (!prodDel) res.status(404).json({ msg: "Error eliminando el producto!" });
